...
| 진입점 | 설명 |
|---|---|
| 키보드(IME) | TypeRight를 기본 키보드로 쓰는 사용자. 아래 트리거 정책대로 실시간 교정 |
| **텍스트 선택 메뉴 (`ACTION_PROCESS_TEXT`)** | 키보드를 바꾸지 않은 사용자도 어떤 앱에서든 문장을 드래그 → 메뉴의 **[맛춤뻡 검사]** → 반투명 창에서 교정. 온디바이스 규칙은 즉시(로그인·동의 불필요), AI는 로그인·동의·150자 이하·한글 2음절 이상일 때만 같은 `/v1/grammar-check` 호출. 수정 가능한 입력란이면 [교정 적용]으로 원문 치환(`setResult`), 읽기 전용이면 복사·짤 생성만 |

### 공유 카드 (📸 짤 생성)
- 키보드 훈수 말풍선·텍스트 선택 검사 창에서 생성. 카톡 말풍선 모양(틀린 부분 표시 + 교정어) + 캐릭터 말풍선(훈수 멘트) + 워터마크 "TypeRight · 맞춤법 훈수 키보드", 1080×1350.
- **개인정보:** 검사한 문장만 들어가고, 전화번호·이메일·주민번호·긴 숫자열은 카드에서 가림 처리.
- 갤러리 저장 + 시스템 공유창(인스타·카톡 등). 인스타 스토리 전용 연동은 Facebook 앱 ID가 필요해 후순위.

### 앱별 피드백 모드
- 우선순위: 사용자 앱별 지정 > 기본 매핑 > 전역 기본 모드. "앱별 자동 모드" 설정(기본 ON)일 때만 기본 매핑 적용.
- 기본 매핑: 메신저·SNS(카카오톡, 인스타그램, 메신저, 디스코드, X, 라인) → 매운맛 / 업무(슬랙, Gmail, Outlook, Teams, 노션, 네이버 메일, 삼성 이메일, 잔디) → 선생님. 경찰 모드는 자동으로 선택되지 않음(사용자 지정만).
- 키보드 제안 바의 모드 칩(🌶️/🚨/🍎)을 탭하면 해당 앱의 모드가 바뀌고 저장됨.
