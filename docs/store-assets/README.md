...
**이대로 출시한다.** 규격 위반이 아니고 심사에서 거부되지 않는다 — 48px 에서 약할 뿐이다.
아이콘은 언제든 교체할 수 있는 항목이므로 이것 때문에 업로드를 미루지 않는다(2026-09-21 결정).
나중에 다듬고 싶으면 이 스크립트의 `BODY`·`CHECK`·`CHECK_WIDTH` 상수만 고쳐 다시 뽑으면 된다
— 체크를 키우거나 본체를 줄이면 작은 크기에서 또렷해진다.
